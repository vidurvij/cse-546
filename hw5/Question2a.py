...
for k in tqdm(range(len(gammas))):
    ...
    for j in tqdm(range(len(lam_vals))):
        ...
        for i in (range(xr.shape[0])):
            ...
            # print(yr.shape)
            exit()
            square = cvx.sum_squares((cvx.matmul(K,alpha)-y))
            constraint = cvx.quad_form(alpha,K)
            # l =  +
            # objective = cvx.Problem(cvx.Minimize(cvx.sum_squares(K*alpha - y) + lam*cvx.quad_form(alpha, K)))
...
# print(losses)
# print(np.argsort(losses.flatten()))
# print(np.sort(losses.flatten()))
plt.legend()
plt.xlabel("X values")
plt.ylabel("Y values")
plt.title("Kernel with Least Squares Loss with $\gamma$ = "+str(gamma))
