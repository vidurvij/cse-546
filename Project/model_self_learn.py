...
def train_model(model,train, valid, criterion, optimizer, num_epochs=3, c = 1):
    ...
    print(total)
    while len(train.dataset) < .8*(total):
        for epoch in tqdm(range(num_epochs)):
            ...
            for i, sample in enumerate(tqdm(train)):
                ...
                inputs, labels, idx = sample['image'], sample['afflictions'], sample["idx"]
                #print(labels)
                ...
                confidence = np.zeros(len(valid))
                index = np.zeros(len(valid))
            for i, sample in enumerate(tqdm(valid)) :
                inputs, labels, idx = sample['image'], sample['afflictions'], sample['idx']
                outputs2 = model(inputs)
                outputs3 = torch.sigmoid(outputs2)
                confidence[len(sample) * i: len(sample)] = outputs3
                index[len(sample) * i: len(sample)] = idx
                #print(outputs)
            ...
            # print(running_corrects)
            changeover = np.argsort(confidence)[0:c]
            for ch in range(c):
                valid.dataset.indices = torch.cat(valid.dataset.indices[0:changeover[c]],valid.dataset.indices[changeover[c]:])
                train.dataset.indices = torch.cat(train.dataset.indices,torch.tensor(changeover[c]))
